	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit'])) {
		...
		*/
		...
		if ($designation == '' || empty($designation)) {
		  $error[] = '<i class="fa fa-star"></i> Designation is required <i class="fa fa-star"></i>';
		}
		...
		*/
		if (!empty($name) && !empty($email) && !empty($password)) {
			...
			if (filter_var($email, FILTER_VALIDATE_EMAIL) === false) {
	            ...
	            $query_check_email 	  = "SELECT * FROM teacher WHERE teacher_email = :email";
		    	...
		    	if (empty($error)) {
					...
					$query 	  = "INSERT INTO teacher(teacher_name, teacher_designation,mobile, teacher_email, teacher_gender,blood_group, teacher_password, teacher_token) 
								VALUES(:name, :designation,:mobile, :email, :gender,:blood_group :password, :token_hash)";
			    	$stmt 	  = $db->prepare($query);

			        $stmt     -> bindValue(':name',$name,PDO::PARAM_STR);
			        $stmt     -> bindValue(':designation',$designation,PDO::PARAM_STR);
			        $stmt     -> bindValue(':mobile',$email,PDO::PARAM_STR);
			        $stmt     -> bindValue(':email',$email,PDO::PARAM_STR);
			        $stmt     -> bindValue(':gender',$gender,PDO::PARAM_STR);
			        $stmt     -> bindValue(':blood_group',$email,PDO::PARAM_STR);			        
	...
	<body class="cbp-spmenu-push">
		...
		<div id="page-wrapper">
			<div class="main-page signup-page">
				<h2 class="title1">Add Teacher</h2>
