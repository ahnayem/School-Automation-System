    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['signin'])) {
        ...
        */
        if (empty($error)) {
            ...
            $query_status           = "SELECT * FROM teacher WHERE teacher_email = :teacher_email AND teacher_status = 1";
            $stmt                   = $db->prepare($query_status);
            $stmt                   -> bindValue(':teacher_email',$email,PDO::PARAM_STR);
            $result                 = $stmt->execute();
            $row                    = $stmt->fetch();

            /*
            * If email matched to the database 
            * and teacher_status is 1
            * then these code will execute
            */
            if ($row) {
                $query_email    = "SELECT * FROM teacher WHERE teacher_email = '$email'";
                ...
                if ($row) {
                        ...
                        if (password_verify($password,$row['teacher_password'])) {
                            ...
                            */
                            if (isset($_POST['remember_me'])) {
                                ...
                                $query    = "INSERT INTO remember_login(token_hash, user_id, expires_at) 
                                            VALUES(:token_hash, :teacher_id, :expires_at)";
                                $stmt     = $db->prepare($query);
                ...
                $error[] = '<i class="fa fa-star"></i> Teacher is not activated! <i class="fa fa-star"></i>';
            }
        ...
        <form action="" method="POST">
            ...
            <div class="forgot">
                <p><input type="checkbox" id="remember_me" name="remember_me" style="position: absolute;">
                    ...
                    <a href="#"><p style="color: yellow;">Forgot Password?</p></a>
            </div>
